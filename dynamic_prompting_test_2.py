...
def main():
    ...
    if True:
        ...
        file_name = "RFCSP_Grounds_Maintenance updated1692707622.txt"
        markdown_path = "D:\Projects\LPMS\markdown_files\RFCSP_Grounds_Maintenance updated1692707622.md"
        ...
        for index, section in enumerate(sections, start = 0):
            ...
            if st.button(":heavy_plus_sign: Suggest Prompts", key=f"Suggest Button {index}") or st.session_state[f"Suggest Button {index}{index}"] == True:
                ...
                if st.button("Use this prompt", key=f"Prompt Button {index} {res.index(selected_prompt)}"):
                    print("calling api")
                    # st.write(f"{selected_prompt} {promptTypes[selected_prompt_type]}")
                    resp = process_request(selected_prompt, section, 0)
                    st.header("Response:")
                    st.write(resp)
                    st.session_state["summary"] += resp + "\n"
                    print(st.session_state["summary"])
            
            if f"Extract Button {index}{index}" not in st.session_state:
                st.session_state[f"Extract Button {index}{index}"] = False

            if st.button(":sparkles: Extract Key Info From This Section", key=f"Extract Button {index}") or st.session_state[f"Extract Button {index}{index}"] == True:
                st.session_state[f"Extract Button {index}{index}"] = True
                    
                promptTypes = {"Bid Submission": 1, "Task": 2, "Locations": 3, "Other" : 0} 
                selected_prompt_type = promptTypes[st.selectbox("Select the type of information:",list(promptTypes.keys()) , key=f"Prompt Type Selectbox {index}")]
                queries = {1: "Extract key details from the text which are important for preparing bid.", 2: "Provide me a list of all the work/task that needs to be performed along with it's frequency for each month and other details", 3: "Provide me a list of all types of locations and other details associated with it. Make sure you don't leave any location."}
                if st.button("Go Ahead",  key=f"Go Button {index} {selected_prompt_type}"):
                    response = process_request(queries[selected_prompt_type], section, selected_prompt_type)
...
def process_request(query, text, promptType):
    ...
    chain = create_chain(text, promptType)
...
def create_chain(text, promptType):
